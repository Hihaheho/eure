...
fn get_completions_with_schema(input: &str, schema_text: &str) -> Vec<CompletionItem> {
    ...
    if let Some(schema) = schema_manager.get_schema("test://schema") {
        eprintln!(
            "DEBUG test: Loaded schema has {} root fields",
...
$type = .any"#;
    ...
    assert!(
        completions.iter().any(|c| c.label == "true"),
        "Should suggest 'true'"
    );
    assert!(
        completions.iter().any(|c| c.label == "false"),
        "Should suggest 'false'"
    );
    assert!(
        completions.iter().any(|c| c.label == "null"),
        "Should suggest 'null'"
    );
}
...
fn test_completion_after_dot() {
    ...
    assert!(
        completions.iter().any(|c| c.label == "version"),
    ...
    assert!(
        !completions
            .iter()
            .any(|c| c.label == "field1" || c.label == "field2"),
        "Should not have dummy fields"
    );
}
...
$type = .object"#;
    ...
    assert!(
        ...
        "Should have completions for partial key"
    );
    assert!(
    ...
    assert!(
        completions.iter().any(|c| c.label == "screen"),
        "Should suggest 'screen'"
    );
    assert!(
        !completions.iter().any(|c| c.label == "settings"),
        "Should not suggest 'settings' (doesn't start with 'scr')"
    );
    assert!(
        completions.iter().all(|c| c.label.starts_with("scr")),
