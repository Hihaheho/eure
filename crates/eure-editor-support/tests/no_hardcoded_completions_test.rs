...
fn test_no_hardcoded_completions_without_schema() {
    ...
    assert!(
        ...
        "Should not have completions without schema"
    );
    assert!(
        !completions.iter().any(|c| c.label == "commands"),
        "Should not have hardcoded 'commands'"
    );
    assert!(
        !completions.iter().any(|c| c.label == "options"),
        "Should not have hardcoded 'options'"
    );
    assert!(
        !completions.iter().any(|c| c.label == "field1"),
        "Should not have hardcoded 'field1'"
    );
    assert!(
        !completions.iter().any(|c| c.label == "field2"),
        "Should not have hardcoded 'field2'"
    );
}
...
fn test_completions_come_from_schema() {
    ...
    assert_eq!(
        completions.len(),
        2,
        "Should have 2 completions from schema"
    );

    let labels: Vec<String> = completions.iter().map(|c| c.label.clone()).collect();
    assert!(
        labels.contains(&"subfield1".to_string()),
        "Should have subfield1 from schema"
    );
    assert!(
        labels.contains(&"subfield2".to_string()),
        "Should have subfield2 from schema"
    );

    // Should not have any hardcoded values
    assert!(
        !labels.contains(&"commands".to_string()),
        "Should not have hardcoded 'commands'"
    );
    assert!(
...
fn test_partial_key_without_schema_returns_empty() {
    ...
    assert!(
        !completions.iter().any(|c| c.label == "script"),
        "Should not have hardcoded 'script'"
    );
}
