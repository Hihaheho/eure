...
"#;
    ...
    let has_missing_field = diagnostics
        .iter()
        .any(|d| d.message.contains("Required field") && d.message.contains("missing"));
    assert!(
        has_missing_field,
...
"#;
    ...
    manager
        .load_schema("test://person.schema.eure", schema_input, &schema_tree)
...
fn test_schema_reference_resolution() {
    ...
    let result =
        schema_validation::resolve_schema_reference(doc_path, "./schemas/config.schema.eure", None);
    assert!(result.is_ok());
    ...
    let result =
        schema_validation::resolve_schema_reference(doc_path, "../shared/types.schema.eure", None);
    assert!(result.is_ok());
    let resolved = result.unwrap();
    ...
    assert!(
        resolved
            .to_string_lossy()
            .contains("shared/types.schema.eure")
    );
    ...
    assert_eq!(
        result.unwrap(),
        Path::new("/etc/schemas/global.schema.eure")
    );
