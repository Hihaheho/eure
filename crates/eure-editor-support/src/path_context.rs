...
use eure_tree::{
    ...
    nodes::{BindingView, SectionView, SectionBodyView, BindingRhsView},
    value_visitor::Values,
    Cst,
};
use eure_value::value::{PathSegment, KeyCmpValue};
...
impl<'a> PathContextExtractor<'a> {
        ...
        for (i, ch) in self.input.char_indices() {
            ...
            if found_start && i >= span.end as usize {
                ...
                return (self.position.line >= start_line && self.position.line <= line) &&
                       (self.position.line > start_line || self.position.character >= start_column) &&
    ...
    fn check_cursor_in_line(&mut self, text: &str, line_offset: u32) {
        ...
        if line_before_cursor.ends_with(':') {
            ...
            is_string_only = true;
            if line_before_cursor.trim_end_matches(':').trim_end().ends_with("$variant") {
