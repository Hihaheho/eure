use eure_tree::{
    ...
    tree::{CstNodeData, CstNodeId, InputSpan, NonTerminalData, TerminalData},
};
...
pub fn get_completions(
    ...
    if let Some(document) = cached_document
        && let Some(handle_completions) =
            get_handle_based_completions(text, cst, position, uri, schema_manager, document)
    {
    ...
    let context = analyze_context_at_position(text, position);
        ...
        CompletionContext::AfterAt {
            ...
            let mut completions = if let Some(variant_name) = variant_context {
                ...
                if is_variant_path(&path, schema) {
                    generate_variant_field_completions(
                        &path,
                        schema,
                        &variant_name,
                        partial_field.as_deref(),
                        &used_fields,
                    )
                } else {
...
fn analyze_context_at_position(text: &str, position: Position) -> CompletionContext {
    ...
    if trimmed.ends_with('@') {
        ...
        if let Some(field_part) = parts.last() {
            ...
            if path.is_empty()
                && let Some(section_path) = find_section_context(text, position)
            {
                path = section_path;
            }
        ...
        if let Some(at_pos) = trimmed.rfind('@') {
            ...
            let partial_field = if after_at.is_empty() {
                None
            } else {
...
fn parse_field_reference(field_ref: &str) -> (Vec<String>, String) {
    ...
    if parts.len() > 1 {
        let path = parts[..parts.len() - 1]
            ...
            .map(|s| s.to_string())
            .collect();
        // Safe: parts.len() > 1, so last() cannot be None
        ...
        if let Some(variant_def) = variant_schema.variants.get(&variant_key) {
            ...
            for (key, field_schema) in &variant_def.fields {
                ...
                if let Some(partial) = partial_field
                    && !field_name.starts_with(partial)
                {
                    continue;
                }
...
fn generate_field_completions(
    ...
    if path.is_empty()
        ...
        && (partial_field.is_none() || "$types".starts_with(partial_field.unwrap_or("")))
    {
        completions.push(CompletionItem {
...
fn generate_variant_completions(path: &[String], schema: &DocumentSchema) -> Vec<CompletionItem> {
    ...
    if let Some(type_field) = schema.types.get(&type_key) {
        ...
        if let Type::Object(type_obj) = &type_field.type_expr {
            ...
            if let Some(variants_field) = type_obj.fields.get(&variants_key)
                && let Type::Variants(variant_schema) = &variants_field.type_expr
            {
                return generate_variant_completion_items(variant_schema);
            }
        }
...
fn lookup_schema_at_path<'a>(path: &[String], root: &'a ObjectSchema) -> Option<&'a ObjectSchema> {
...
fn find_used_fields_in_section(
    text: &str,
    position: Position,
    section_path: &[String],
) -> HashSet<String> {
    ...
    let normalized_path: Vec<String> = section_path.iter().map(|s| s.replace("[]", "")).collect();
...
fn find_variant_context(text: &str, position: Position, section_path: &[String]) -> Option<String> {
    ...
    let section_pattern = if section_path.is_empty() {
        ...
        let normalized_path: Vec<String> =
            section_path.iter().map(|s| s.replace("[]", "")).collect();
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if in_section && i <= position.line as usize {
            ...
            if trimmed.starts_with('@') && section_pattern.is_some() {
                let matches_pattern = section_pattern
                    .as_ref()
                    .map(|p| {
                        trimmed == p
                            || trimmed == format!("{}[]", p)
                            || trimmed.starts_with(&p.to_string())
                    })
                    .unwrap_or(false);
...
fn find_node_at_position_recursive(
    ...
    if let Some(span) = get_node_span(cst, node_id)
        && (position_byte < span.start as usize || position_byte > span.end as usize)
    {
...
fn get_handle_based_completions(
    ...
    if document.get_node_by_cst_id(cst_node_id).is_some() {
        ...
        if let Some(path) = document.get_path_by_cst_id(cst_node_id) {
            ...
            let context = analyze_handle_completion_context(byte_offset, text);

            // Generate completions based on context
            match context {
                HandleCompletionContext::FieldPosition => {
                    // Complete field names at the current location
                    Some(generate_field_completions(
                        &string_path,
                        schema,
                        None,
                        &HashSet::new(),
                ...
                HandleCompletionContext::ValuePosition { field_name } => {
                    ...
                    if field_name == "variant" {
                        ...
                        Some(generate_value_completions(
                            &string_path,
                            &field_name,
                            schema,
                            false,
        ...
        if let Some(parent_path) = document.get_path_by_cst_id(parent_cst_id) {
            // Convert PathSegment to String for compatibility
            let string_path: Vec<String> = parent_path
                .iter()
                .filter_map(|seg| {
                    match seg {
                        eure_value::value::PathSegment::Ident(id) => Some(id.as_ref().to_string()),
...
enum HandleCompletionContext {
    ...
    ValuePosition { field_name: String },
...
fn analyze_handle_completion_context(byte_offset: usize, text: &str) -> HandleCompletionContext {
...
fn extract_field_name_before_operator(text: &str) -> Option<String> {
    ...
    let parts: Vec<&str> = without_operator.split_whitespace().collect();
