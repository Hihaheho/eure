...
use crate::{value_to_json, json_to_value, value_to_json_with_config, json_to_value_with_config};
...
fn test_dollar_tag_preservation() {
    ...
    map.insert(KeyCmpValue::String("$tag".to_string()), Value::String("TestVariant".to_string()));
    map.insert(KeyCmpValue::String("field".to_string()), Value::String("value".to_string()));
...
fn test_basic_type_conversions() {
    ...
    // String
    assert_eq!(to_json_string(&Value::String("hello".to_string())).unwrap(), "\"hello\"");
    assert_eq!(from_json_string("\"world\"").unwrap(), Value::String("world".to_string()));
...
fn test_deeply_nested_structures() {
    ...
    inner.insert(KeyCmpValue::String("nested".to_string()), Value::Map(Map(innermost)));
    
    let mut outer = AHashMap::new();
    outer.insert(KeyCmpValue::String("data".to_string()), Value::Map(Map(inner)));
    outer.insert(KeyCmpValue::String("array".to_string()), 
...
fn test_empty_and_whitespace_keys() {
    ...
    // Empty key
    let mut map = AHashMap::new();
    map.insert(KeyCmpValue::String("".to_string()), Value::String("empty key".to_string()));
    map.insert(KeyCmpValue::String(" ".to_string()), Value::String("space key".to_string()));
    map.insert(KeyCmpValue::String("\t".to_string()), Value::String("tab key".to_string()));
...
fn test_variant_representations() {
    use eure_value::value::{Value, Variant, Map, KeyCmpValue};
    use ahash::AHashMap;
    use crate::Config;
    ...
    let configs = vec![
        Config { variant_repr: crate::VariantRepr::External },
        Config { variant_repr: crate::VariantRepr::Internal { tag: "type".to_string() } },
        Config { variant_repr: crate::VariantRepr::Adjacent { tag: "t".to_string(), content: "c".to_string() } },
        Config { variant_repr: crate::VariantRepr::Untagged },
    ];
...
fn test_tuple_conversions() {
    use eure_value::value::{Value, Tuple};
