...
"#;
    ...
    println!("Global rename-all: {:?}", schema.document_schema.serde_options.rename_all);
    
    // Check type-specific rename-all
    if let Some(person_type) = schema.document_schema.types.get(&eure_value::value::KeyCmpValue::String("Person".to_string())) {
