...
use eure_tree::document::{DocumentKey, EureDocument, Node, NodeContent};
use eure_value::value::{KeyCmpValue, PathSegment, PathKey, Path};
...
pub fn document_to_schema(doc: &EureDocument) -> Result<DocumentSchema, SchemaError> {
    ...
    if let Some(schema_node_id) = root.extensions.get(&Identifier::from_str("schema").unwrap()) {
        ...
        if let NodeContent::String { value: schema_ref, .. } = &schema_node.content {
    ...
    if let Some(cascade_node_id) = root.extensions.get(&Identifier::from_str("cascade-type").unwrap()) {
        ...
        if let NodeContent::Path { value: path, .. } = &cascade_node.content {
...
fn is_schema_or_nested_schema_node(doc: &EureDocument, node: &Node) -> bool {
    ...
    match &node.content {
        NodeContent::Map { .. } => is_pure_schema_node(doc, node),
        _ => false,
    }
}
...
impl SchemaBuilder {
        ...
        for (ext_name, ext_node_id) in &node.extensions {
            ...
            match ext_name.as_ref() {
                ...
                "cascade-type" => {
                    ...
                    if let NodeContent::Path { value: type_path, .. } = &ext_node.content {
            ...
            NodeContent::Map { .. } => {
                ...
                if let Some(variants_node_id) = node.extensions.get(&Identifier::from_str("variants").unwrap()) {
                    ...
                    if let Some(repr_node_id) = node.extensions.get(&Identifier::from_str("variant.repr").unwrap()) {
                        ...
                        if let NodeContent::String { value: repr_str, .. } = &repr_node.content {
                    ...
                    return Ok(Some(FieldSchema {
                        type_expr: Type::Variant(VariantSchema {
                            variants,
                            representation: repr,
                        }),
                        ..Default::default()
    ...
    fn extract_field_schema_from_node(&self, doc: &EureDocument, _field_name: &str, node: &Node) -> Result<Option<FieldSchema>, SchemaError> {
        ...
        for (ext_name, ext_node_id) in &node.extensions {
            ...
            match ext_name.as_ref() {
                "type" => {
                    if let NodeContent::Path { value: path, .. } = &ext_node.content {
                        if let Some(type_expr) = Type::from_path_segments(&path.0) {
                ...
                "optional" => {
                    if let NodeContent::Bool { value: b, .. } = &ext_node.content {
                        schema.optional = *b;
                        has_schema = true;
                    }
                }
                "min" => {
                    schema.min = Self::extract_f64_from_node(&ext_node.content);
                    if schema.min.is_some() {
                        has_schema = true;
                    }
                }
                "max" => {
                    schema.max = Self::extract_f64_from_node(&ext_node.content);
                    if schema.max.is_some() {
                        has_schema = true;
                    }
                }
                "min-length" => {
                    schema.min_length = Self::extract_usize_from_node(&ext_node.content);
                    if schema.min_length.is_some() {
                        has_schema = true;
                    }
                }
                "max-length" => {
                    schema.max_length = Self::extract_usize_from_node(&ext_node.content);
                    if schema.max_length.is_some() {
                        has_schema = true;
                    }
                }
                "pattern" => {
                    if let NodeContent::String { value: s, .. } = &ext_node.content {
                        schema.pattern = Some(s.clone());
                        has_schema = true;
                    }
                }
                "values" => {
                    if let NodeContent::Array { children, .. } = &ext_node.content {
                        let mut allowed_values = Vec::new();
                        for value_node_id in children {
                            let value_node = doc.get_node(*value_node_id);
                            if let Some(val) = Self::node_content_to_value(&value_node.content) {
        ...
        match &node.content {
            ...
            _ if has_schema => {
                ...
                if let Some(val) = Self::node_content_to_value(&node.content) {
                    schema.default = Some(val);
                }
            }
            _ => {}
        }
    ...
    fn extract_variant_schema(&self, doc: &EureDocument, _variant_name: &str, variant_node: &Node) -> Result<ObjectSchema, SchemaError> {
        ...
        if let NodeContent::Map { .. } = &variant_node.content {
    ...
    fn extract_usize_from_node(content: &NodeContent) -> Option<usize> {
        match content {
            NodeContent::I64 { value, .. } => Some(*value as usize),
            NodeContent::U64 { value, .. } => Some(*value as usize),
            _ => None,
        }
    }
    
    /// Helper to extract f64 from NodeContent
    fn extract_f64_from_node(content: &NodeContent) -> Option<f64> {
        match content {
            NodeContent::I64 { value, .. } => Some(*value as f64),
            NodeContent::U64 { value, .. } => Some(*value as f64),
            NodeContent::F32 { value, .. } => Some(*value as f64),
            NodeContent::F64 { value, .. } => Some(*value),
            _ => None,
        }
    }
    
    /// Convert NodeContent to Value for schema purposes
    fn node_content_to_value(content: &NodeContent) -> Option<eure_value::value::Value> {
        use eure_value::value::{Value, Code as ValueCode, Array, Map as ValueMap, Tuple};
        use ahash::AHashMap;
        
        match content {
            NodeContent::Null { .. } => Some(Value::Null),
            NodeContent::Bool { value, .. } => Some(Value::Bool(*value)),
            NodeContent::I64 { value, .. } => Some(Value::I64(*value)),
            NodeContent::U64 { value, .. } => Some(Value::U64(*value)),
            NodeContent::F32 { value, .. } => Some(Value::F32(*value)),
            NodeContent::F64 { value, .. } => Some(Value::F64(*value)),
            NodeContent::String { value, .. } => Some(Value::String(value.clone())),
            NodeContent::Code { value, .. } => Some(Value::Code(ValueCode {
                language: value.language.clone(),
                content: value.content.clone(),
            })),
            NodeContent::CodeBlock { value, .. } => Some(Value::CodeBlock(ValueCode {
                language: value.language.clone(),
                content: value.content.clone(),
            })),
            NodeContent::NamedCode { value, .. } => Some(Value::Code(ValueCode {
                language: value.language.clone(),
                content: value.content.clone(),
            })),
            NodeContent::Path { value, .. } => Some(Value::Path(value.clone())),
            NodeContent::Hole { .. } => Some(Value::Hole),
            // For complex types, we'll return None as they require recursive conversion
            NodeContent::Array { .. } | NodeContent::Map { .. } | NodeContent::Tuple { .. } => None,
        }
    }
    ...
    fn extract_serde_options_from_entries(doc: &EureDocument, entries: &[(DocumentKey, eure_tree::document::NodeId)]) -> SerdeOptions {
        ...
        for (key, node_id) in entries {
            if let DocumentKey::Ident(ident) = key {
                ...
                match ident.as_ref() {
                    "rename" => {
                        if let NodeContent::String { value: rename, .. } = &node.content {
                            options.rename = Some(rename.clone());
                        }
                    }
                    "rename-all" => {
                        if let NodeContent::String { value: rename_all, .. } = &node.content {
