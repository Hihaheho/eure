...
"#;
    ...
    if let Value::Map(map) = doc {
        ...
        if let Some(Value::Path(Path(segments))) = map.0.get(&KeyCmpValue::String("test1".to_string())) {
        ...
        if let Some(Value::Path(Path(segments))) = map.0.get(&KeyCmpValue::String("test2".to_string())) {
            assert_eq!(segments.len(), 2);
        ...
        if let Some(Value::Path(Path(segments))) = map.0.get(&KeyCmpValue::String("test3".to_string())) {
            assert_eq!(segments.len(), 2);
            assert!(matches!(&segments[0], PathSegment::MetaExt(id) if id.to_string() == "meta"));
            assert!(matches!(&segments[1], PathSegment::Ident(id) if id.to_string() == "extension"));
