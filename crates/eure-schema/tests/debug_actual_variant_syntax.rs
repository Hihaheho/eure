use eure_schema::{document_to_schema, validate_document, Type};
...
"#;
    ...
    tree1.visit_from_root(&mut visitor1).expect("Failed to visit");
...
"#;
    ...
    test_tree.visit_from_root(&mut test_visitor).expect("Failed to visit doc");
...
"#;
    ...
    tree2.visit_from_root(&mut visitor2).expect("Failed to visit");
    let doc2 = visitor2.into_document();
    let schema2 = document_to_schema(&doc2).expect("Failed to extract schema");
    
    if let Some(msg_type) = schema2.types.get(&KeyCmpValue::String("Message".to_string())) {
...
"#;
    ...
    tree3.visit_from_root(&mut visitor3).expect("Failed to visit");
