...
"#;
    ...
    if let Value::Map(map) = doc {
        ...
        assert!(
            map.0
                .contains_key(&KeyCmpValue::String("root1".to_string())),
            "root1 should be at root"
        );
        assert!(
            map.0
                .contains_key(&KeyCmpValue::String("root2".to_string())),
            "root2 should be at root"
        );
        assert!(
            map.0
                .contains_key(&KeyCmpValue::String("section1".to_string())),
            "section1 should be at root"
        );
...
"#;
    ...
    if let Value::Map(map) = doc {
        ...
        if let Some(Value::Map(empty_map)) =
            map.0.get(&KeyCmpValue::String("empty_section".to_string()))
        {
            assert!(
                empty_map.0.is_empty(),
                "empty_section should be an empty map"
            );
