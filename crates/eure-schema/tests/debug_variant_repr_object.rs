...
"#;
    ...
    if let Some(event_type) = schema.types.get(&KeyCmpValue::String("Event".to_string())) {
        if let Type::Variants(variant_schema) = &event_type.type_expr {
            println!("Extracted representation: {:?}", variant_schema.representation);
...
"#;
    ...
    schema_tree2.visit_from_root(&mut schema_visitor2).expect("Failed to visit schema tree");
    let schema_doc2 = schema_visitor2.into_document();
    let schema2 = document_to_schema(&schema_doc2).expect("Failed to extract schema");
    
    if let Some(message_type) = schema2.types.get(&KeyCmpValue::String("Message".to_string())) {
...
"#;
    ...
    schema_tree3.visit_from_root(&mut schema_visitor3).expect("Failed to visit schema tree");
