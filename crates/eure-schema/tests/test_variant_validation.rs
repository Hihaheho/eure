...
"#;
    ...
    assert_eq!(
        errors.len(),
        0,
        "Valid variant should have no errors, but got: {errors:?}"
    );
...
"#;
    ...
    assert!(
        has_unknown_variant || !errors5.is_empty(),
...
"#;
    ...
    tree6
        .visit_from_root(&mut visitor6)
        .expect("Failed to visit tree");
    let document6 = visitor6.into_document();
    let errors6 = validate_document(&document6, &schema);

    let has_missing_tag = errors6
        .iter()
        .any(|e| matches!(&e.kind, ValidationErrorKind::UnknownVariant { .. }));

    assert!(
        has_missing_tag || !errors6.is_empty(),
...
"#;
    ...
    // Check that valid fields are not reported as unexpected
    let unexpected_errors2: Vec<_> = errors2
        .iter()
        .filter(|e| matches!(&e.kind, ValidationErrorKind::UnexpectedField { .. }))
        .collect();

    assert_eq!(
        unexpected_errors2.len(),
        0,
...
"#;
    ...
    let unexpected_errors: Vec<_> = errors2
        .iter()
        .filter(|e| matches!(&e.kind, ValidationErrorKind::UnexpectedField { .. }))
        .collect();
    ...
    assert!(
        has_invalid_field,
        "Should report 'invalid_field' as unexpected"
    );
    assert!(
        has_another_invalid,
        "Should report 'another_invalid' as unexpected"
    );
}
