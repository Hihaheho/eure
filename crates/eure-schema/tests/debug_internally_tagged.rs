use eure_schema::{document_to_schema, validate_document, Type, VariantSchema};
...
"#;
    ...
    if let Some(event_type) = schema.types.get(&KeyCmpValue::String("Event".to_string())) {
        if let Type::Variants(variant_schema) = &event_type.type_expr {
            println!("Variant representation: {:?}", variant_schema.representation);
...
"#;
    ...
    tree1.visit_from_root(&mut visitor1).expect("Failed to visit tree");
