...
"#;
    ...
    if let Value::Map(map) = doc_value {
        ...
        if map
            .0
            .contains_key(&eure_value::value::KeyCmpValue::String("users".to_string()))
        {
            ...
            if let Some(Value::Map(types_map)) = map.0.get(&eure_value::value::KeyCmpValue::String(
                "$types".to_string(),
            )) && let Some(Value::Map(user_map)) = types_map
                .0
                .get(&eure_value::value::KeyCmpValue::String("User".to_string()))
            {
                println!(
                    "User type keys: {:?}",
                    user_map.0.keys().collect::<Vec<_>>()
                );
            }
        }
    }
...
"#;
    ...
    if let Value::Map(map) = doc_value {
        println!(
            "\nInline syntax - Top-level keys: {:?}",
            map.0.keys().collect::<Vec<_>>()
        );
        assert!(
            map.0
                .contains_key(&eure_value::value::KeyCmpValue::String("users".to_string())),
