...
fn test_string_constraints() {
    ...
    if let Type::Object(obj_schema) = &schema.type_expr {
        ...
        let email_field = &obj_schema.fields["email"];
...
fn test_number_constraints() {
    ...
    if let Type::Object(obj_schema) = &schema.type_expr {
        let price_field = &obj_schema.fields["price"];
        assert_eq!(price_field.constraints.range, Some((Some(0.0), Some(1000000.0))));
        
        let quantity_field = &obj_schema.fields["quantity"];
...
fn test_array_constraints() {
    ...
    if let Type::Object(obj_schema) = &schema.type_expr {
        let tags_field = &obj_schema.fields["tags"];
        ...
        let items_field = &obj_schema.fields["items"];
...
fn test_preferences() {
    ...
    if let Type::Object(obj_schema) = &schema.type_expr {
        let metadata_field = &obj_schema.fields["metadata"];
        assert_eq!(metadata_field.preferences.section, Some(true));
        
        let simple_field = &obj_schema.fields["simple_data"];
...
fn test_combined_attributes() {
    ...
    if let Type::Object(obj_schema) = &schema.type_expr {
        ...
        assert!(obj_schema.fields.contains_key("user_id"));
        let id_field = &obj_schema.fields["user_id"];
        assert_eq!(id_field.constraints.pattern, Some("^[A-Z0-9]{8}$".to_string()));
        assert_eq!(id_field.serde.rename, Some("user_id".to_string()));
        
        // Check password field
        let password_field = &obj_schema.fields["password"];
        ...
        let balance_field = &obj_schema.fields["balance"];
