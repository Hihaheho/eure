...
use eure_schema::{ToEureSchema, Type, KeyCmpValue};
...
fn test_range_with_integer_literals() {
    ...
    if let Type::Object(obj_schema) = schema.type_expr {
        let age_field = obj_schema.fields.get(&KeyCmpValue::String("age".to_string())).unwrap();
...
fn test_range_with_float_literals() {
    ...
    if let Type::Object(obj_schema) = schema.type_expr {
        let score_field = obj_schema.fields.get(&KeyCmpValue::String("score".to_string())).unwrap();
...
fn test_range_with_mixed_literals() {
    ...
    if let Type::Object(obj_schema) = schema.type_expr {
        let value_field = obj_schema.fields.get(&KeyCmpValue::String("value".to_string())).unwrap();
...
fn test_range_with_negative_integers() {
    ...
    if let Type::Object(obj_schema) = schema.type_expr {
        let temp_field = obj_schema.fields.get(&KeyCmpValue::String("temperature".to_string())).unwrap();
        assert_eq!(temp_field.constraints.range, Some((Some(-100.0), Some(100.0))));
    } else {
        panic!("Expected object schema");
    }
}