...
mod visitor_tests {
    ...
    use crate::nodes::{
        ExtHandle, ExtensionNameSpaceHandle, ExtensionNameSpaceView, Ident, IdentView,
        KeyBaseHandle, KeyBaseView, KeyOptHandle, KeyView, ValueView,
    };
    use crate::prelude::*;
    use crate::tree::{CstFacade, CstNodeData, InputSpan, NonTerminalData, TerminalData};
    ...
    impl MockTree {
        ...
        fn add_terminal(
            ...
            self.node_data
                .insert(node_id, CstNodeData::Terminal { kind, data });
            node_id
        }
    }
    ...
    impl CstFacade for MockTree {
        fn get_str<'a: 'c, 'b: 'c, 'c>(
            ...
            match terminal {
                TerminalData::Input(span) => {
                    Some(&self.input[span.start as usize..span.end as usize])
                }
                _ => None,
            }
        }
        ...
        fn get_terminal(
            ...
            node: CstNodeId,
            kind: TerminalKind,
        ) -> Result<TerminalData, crate::CstConstructError> {
            self.terminals
                .get(&(node, kind))
                .copied()
        ...
        ) -> Result<NonTerminalData, crate::CstConstructError> {
        ...
        fn collect_nodes<
            'v,
            const N: usize,
            V: crate::visitor::BuiltinTerminalVisitor<E, Self>,
            O,
            E,
        >(
            &self,
            ...
            _visitor: impl FnMut(
                [CstNodeId; N],
                &'v mut V,
            ) -> Result<(O, &'v mut V), crate::CstConstructError<E>>,
    ...
    fn test_visit_null() {
        ...
        visitor
            .visit_value(value_handle, ValueView::Null(null_handle), &tree)
            .unwrap();
    ...
    fn test_visit_identifier() {
        ...
        visitor
            .visit_ident(ident_handle, ident_view, &tree)
            .unwrap();
    ...
    fn test_value_construction() {
        ...
        map_data.insert(
            KeyCmpValue::String("key".to_string()),
    ...
    fn test_extension_namespace() {
        ...
        visitor
            .visit_ident(IdentHandle(ident_node), ident_view, &tree)
            .unwrap();
        ...
        assert!(
            matches!(path_segment, PathSegment::Extension(ref ident) if ident.to_string() == "eure")
        );
    }
    ...
    fn test_array_marker_parsing() {
        // Test array marker with index
        {
            ...
            visitor
                .visit_ident(IdentHandle(key_ident_node), ident_view, &tree)
                .unwrap();
    ...
    fn test_collect_object_items() {
        ...
        visitor
            .visit_ident(
                IdentHandle(key1_node),
                IdentView {
                    ident: Ident(key1_node),
                },
                &tree,
            )
            .unwrap();
        visitor
            .visit_ident(
                IdentHandle(key2_node),
                IdentView {
                    ident: Ident(key2_node),
                },
                &tree,
            )
            .unwrap();
    ...
    fn test_complex_key_parsing() {
        ...
        assert!(matches!(
            string_segment,
            PathSegment::Value(KeyCmpValue::String(_))
        ));
        assert!(matches!(
            int_segment,
            PathSegment::Value(KeyCmpValue::I64(_))
        ));
        assert!(matches!(array_segment, PathSegment::Array { .. }));
        assert!(matches!(
            array_no_index,
            PathSegment::Array { index: None, .. }
        ));
    }
}
