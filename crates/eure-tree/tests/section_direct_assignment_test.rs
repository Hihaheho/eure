...
"#;
    ...
    if let NodeValue::Map { entries, .. } = &root.content {
        ...
        let section1_node = entries.iter()
        ...
        let section2_node = entries.iter()
        ...
        let section3_node = entries.iter()
        ...
        let section4_node = entries.iter()
...
"#;
    ...
    if let NodeValue::Map { entries, .. } = &root.content {
        ...
        let config_node = entries.iter()
            .find(|(k, _)| k == &config_key)
            .map(|(_, id)| document.get_node(*id))
            .expect("config should exist");
        
        if let NodeValue::Map { entries: config_entries, .. } = &config_node.content {
            // Check database
            let database_key = DocumentKey::Ident(Identifier::from_str("database").unwrap());
            let database_node = config_entries.iter()
                .find(|(k, _)| k == &database_key)
                .map(|(_, id)| document.get_node(*id))
                .expect("database should exist");
            
            if let NodeValue::Map { entries: db_entries, .. } = &database_node.content {
                // Check host
                let host_key = DocumentKey::Ident(Identifier::from_str("host").unwrap());
                let host_node = db_entries.iter()
                    .find(|(k, _)| k == &host_key)
                    .map(|(_, id)| document.get_node(*id))
            ...
            let server_node = config_entries.iter()
                .find(|(k, _)| k == &server_key)
                .map(|(_, id)| document.get_node(*id))
                .expect("server should exist");
            
            if let NodeValue::Map { entries: server_entries, .. } = &server_node.content {
                let enabled_key = DocumentKey::Ident(Identifier::from_str("enabled").unwrap());
                let enabled_node = server_entries.iter()
...
"#;
    ...
    if let NodeValue::Map { entries, .. } = &root.content {
        ...
        let items_node = entries.iter().find(|(k, _)| k == &items_key).map(|(_, id)| id)
            .map(|id| document.get_node(*id))
            .expect("items should exist");
        
        if let NodeValue::Array { children: elements, .. } = &items_node.content {
...
"#;
    ...
    if let NodeValue::Map { entries, .. } = &root.content {
        ...
        let user_node = entries.iter().find(|(k, _)| k == &user_key).map(|(_, id)| id)
            .map(|id| document.get_node(*id))
            .expect("user should exist");
        
        // Check $type extension
        let type_ext = user_node.extensions.get(&Identifier::from_str("type").unwrap())
            .map(|id| document.get_node(*id))
        ...
        let variant_ext = user_node.extensions.get(&Identifier::from_str("variant").unwrap())
            .map(|id| document.get_node(*id))
        ...
        let config_key = DocumentKey::Ident(Identifier::from_str("config").unwrap());
        let config_node = entries.iter().find(|(k, _)| k == &config_key).map(|(_, id)| id)
            .map(|id| document.get_node(*id))
            .expect("config should exist");
        
        // Meta-extensions are stored as entries in the Map, not in extensions
        if let NodeValue::Map { entries: config_entries, .. } = &config_node.content {
            let internal_key = DocumentKey::MetaExtension(Identifier::from_str("internal").unwrap());
            let internal_meta = config_entries.iter()
...
"#;
    ...
    if let NodeValue::Map { entries, .. } = &root.content {
        ...
        let data_node = entries.iter().find(|(k, _)| k == &data_key).map(|(_, id)| id)
            .map(|id| document.get_node(*id))
            .expect("data should exist");
        
        if let NodeValue::Map { entries: data_entries, .. } = &data_node.content {
            // Check object
            let object_key = DocumentKey::Ident(Identifier::from_str("object").unwrap());
            let object_node = data_entries.iter().find(|(k, _)| k == &object_key).map(|(_, id)| id)
                .map(|id| document.get_node(*id))
                .expect("object should exist");
            
            if let NodeValue::Map { entries: obj_entries, .. } = &object_node.content {
                assert_eq!(obj_entries.len(), 2);
                
                let key_key = DocumentKey::Ident(Identifier::from_str("key").unwrap());
                let key_node = obj_entries.iter().find(|(k, _)| k == &key_key).map(|(_, id)| id)
                    .map(|id| document.get_node(*id))
            ...
            let array_node = data_entries.iter().find(|(k, _)| k == &array_key).map(|(_, id)| id)
                .map(|id| document.get_node(*id))
                .expect("array should exist");
            
            if let NodeValue::Array { children: elements, .. } = &array_node.content {
                assert_eq!(elements.len(), 3);
            }
            
            // Check tuple
            let tuple_key = DocumentKey::Ident(Identifier::from_str("tuple").unwrap());
            let tuple_node = data_entries.iter().find(|(k, _)| k == &tuple_key).map(|(_, id)| id)
                .map(|id| document.get_node(*id))
                .expect("tuple should exist");
            
            if let NodeValue::Tuple { children: elements, .. } = &tuple_node.content {
                assert_eq!(elements.len(), 3);
            }
        }
    }
...
"#;
    ...
    if let NodeValue::Map { entries, .. } = &root.content {
        ...
        if let NodeValue::Map { entries: config_entries, .. } = &config_node.content {
            ...
            assert!(config_entries.iter().any(|(k, _)| k == &name_key), "name should exist");
            
            // Check database (traditional section)
            let database_key = DocumentKey::Ident(Identifier::from_str("database").unwrap());
            let database_node = config_entries.iter().find(|(k, _)| k == &database_key).map(|(_, id)| id)
                .map(|id| document.get_node(*id))
                .expect("database should exist");
            
            if let NodeValue::Map { entries: db_entries, .. } = &database_node.content {
                assert!(db_entries.iter().any(|(k, _)| k == &DocumentKey::Ident(Identifier::from_str("host").unwrap())));
                assert!(db_entries.iter().any(|(k, _)| k == &DocumentKey::Ident(Identifier::from_str("port").unwrap())));
            }
            
            // Check cache (direct assignment after traditional section)
            let cache_key = DocumentKey::Ident(Identifier::from_str("cache").unwrap());
            assert!(config_entries.iter().any(|(k, _)| k == &cache_key), "cache should exist");
        }
    }
}