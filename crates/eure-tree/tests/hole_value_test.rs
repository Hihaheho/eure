    ...
    match doc_value {
        Value::Map(map) => {
                ...
                .get(&eure_value::value::KeyCmpValue::String("items".to_string()))
            {
                ...
                assert!(
                    matches!(&items.0[0], Value::Hole),
                    "Expected first item to be a hole"
                );
                assert!(
                    matches!(&items.0[1], Value::String(s) if s == "second"),
                    "Expected second item to be 'second'"
                );
                assert!(
                    matches!(&items.0[2], Value::Hole),
                    "Expected third item to be a hole"
                );
            } else {
...
fn test_hole_in_different_contexts() {
    ...
    for (input, description) in test_cases {
        let tree =
            eure_parol::parse(input).unwrap_or_else(|_| panic!("Failed to parse: {description}"));

        let mut visitor = eure_tree::value_visitor::ValueVisitor::new(input);
        tree.visit_from_root(&mut visitor)
            .unwrap_or_else(|_| panic!("Failed to visit tree: {description}"));
    ...
    match doc_value {
        Value::Map(map) => {
            ...
            assert!(
                ...
                "Expected test value to be a hole, got {test_value:?}"
            );
        }
        _ => panic!("Expected document to be a map"),
    }
