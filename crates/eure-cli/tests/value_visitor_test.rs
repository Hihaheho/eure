...
age = 30"#;
    ...
    let first_binding = bindings_view
        .binding
        .get_view(&tree)
        .expect("Failed to get first binding");

    // Verify key
    let key_handles = values
        .get_keys(&first_binding.keys)
        .expect("Failed to get keys for first binding");
    assert_eq!(
        key_handles.len(),
        1,
        "Expected exactly one key for 'name' binding"
    );

    let path_seg = values
        .get_path_segment(&key_handles[0])
    ...
    // Verify key
    let key_handles = values
        .get_keys(&second_binding.keys)
        .expect("Failed to get keys for second binding");
    assert_eq!(
        key_handles.len(),
        1,
        "Expected exactly one key for 'age' binding"
    );
    ...
    // Verify value
    let binding_rhs = second_binding
        .binding_rhs
        .get_view(&tree)
        .expect("Failed to get binding rhs");
    match binding_rhs {
...
items = ["apple", "banana"]"#;
    ...
    assert_eq!(
        key_handles.len(),
        1,
        "Expected exactly one key for 'user' binding"
    );
    ...
    assert_eq!(
        key_handles.len(),
        1,
        "Expected exactly one key for 'items' binding"
    );
...
email = "test@example.com""#;
    ...
    let root_binding = bindings_view
        .binding
        .get_view(&tree)
    ...
    // Check sections
    let sections_view = eure_view
        .eure_sections
        .get_view(&tree)
        .expect("Failed to get sections")
        .expect("No sections found");

    let section = sections_view
        .section
        .get_view(&tree)
        .expect("Failed to get section");

    // Verify section key
    let section_keys = values
        .get_keys(&section.keys)
        .expect("Failed to get section keys");
    assert_eq!(
        section_keys.len(),
        1,
        "Expected exactly one key for section"
    );

    let section_path_seg = values
        .get_path_segment(&section_keys[0])
    ...
    match section_body {
        SectionBodyView::SectionBodyList(section_body_list_handle) => {
            let section_body_list = section_body_list_handle
                .get_view(&tree)
                .expect("Failed to get section body list")
                .expect("No section body list found");

            // First binding in section (id = 456)
            let first_section_binding = section_body_list
                .binding
                .get_view(&tree)
                .expect("Failed to get first section binding");

            let id_keys = values
                .get_keys(&first_section_binding.keys)
                .expect("Failed to get keys for id binding");
            assert_eq!(
                id_keys.len(),
                1,
                "Expected exactly one key for 'id' binding"
            );

            let id_path_seg = values
                .get_path_segment(&id_keys[0])
...
port = 5432"#;
    ...
    assert_eq!(
        section_keys.len(),
        2,
        "Expected two keys for nested section"
    );

    // First key should be "config"
    let first_path_seg = values
        .get_path_segment(&section_keys[0])
    ...
    let second_path_seg = values
        .get_path_segment(&section_keys[1])
...
nothing = null"#;
    ...
    fn check_binding_value(
        ...
        // Check key
        let key_handles = values.get_keys(&binding.keys).expect("Failed to get keys");
        ...
        match binding_rhs {
            BindingRhsView::ValueBinding(value_binding_handle) => {
                ...
                match (expected_value, value) {
                    ...
                    _ => panic!(
                        "Unexpected value type or value: expected {expected_value}, got {value:?}"
                    ),
                }
            }
    ...
    // Check nothing = null
    let third_bindings = second_bindings
        .eure_bindings
        .get_view(&tree)
        .expect("Failed to get remaining bindings")
        .expect("No third binding found");
    let third_binding = third_bindings
        .binding
        .get_view(&tree)
