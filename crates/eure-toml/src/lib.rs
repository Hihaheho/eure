...
pub fn format_source_document(source: &SourceDocument) -> String {
    let mut formatter = SourceFormatter::new(&source.document, &source.layout);
...
impl<'a> SourceFormatter<'a> {
    fn new(doc: &'a eure_document::document::EureDocument, layout: &'a Layout) -> Self {
        Self {
            doc,
            layout,
            output: String::new(),
    ...
    fn format_item(&mut self, item: &LayoutItem, indent: usize) {
        match item {
            ...
            LayoutItem::ArrayBinding {
                ...
                self.format_path(path);
                self.output.push_str(" = ");
                self.format_array_with_comments(*node, elements, indent);
                if let Some(comment) = trailing_comment {
                    self.output.push_str(" //");
                    if !comment.is_empty() {
                        self.output.push(' ');
        ...
        match &node.content {
            ...
            NodeValue::Primitive(prim) => match prim {
                ...
                PrimitiveValue::Text(text) => {
                    ...
                    // Check if this is a block code based on syntax_hint
                    ...
                    if is_block {
                        // Block code - output with appropriate backticks
                        ...
                        self.output.push_str(backticks);
                        // Add language tag if not implicit/plaintext
    ...
    /// Format an array with per-element comments as a multi-line array.
    fn format_array_with_comments(
        &mut self,
        node_id: NodeId,
        ...
        if let NodeValue::Array(arr) = &node.content {
            ...
            for (i, &child_id) in arr.iter().enumerate() {
                // Output comments before this element
                ...
                // Output trailing comment if present (on same line)
                if let Some(el) = elements.iter().find(|e| e.index == i)
