...
impl ConstructorGenerator {
    ...
    fn generate_sequence_constructor(
        ...
        let fields: Vec<_> = nt.children.iter().enumerate().map(|(idx, child)| {
        ...
        let embed_children = fields.iter().map(|(name, _)| {
            quote!(let #name = builder.embed(self.#name.builder);)
        });
    ...
    fn generate_one_of_constructor(
        ...
        let variants: Vec<_> = nt.children.iter().map(|child| {
            match &child.name {
                NodeName::Terminal(term) => {
                    let term_info = self.get_terminal_by_name(node_info, &term.0);
        ...
        let variant_matches = variants.iter().map(|(variant, _)| {
            quote!(Self::#variant(node) => builder.embed(node.builder))
        });
    ...
    fn derive_field_name(&self, _node_info: &NodeTypesInfo, nt: &NonTerminalInfo, idx: usize) -> String {
    ...
    fn generate_terminal_constructors(&self, node_info: &NodeTypesInfo) -> TokenStream {
        let constructors: Vec<_> = node_info
            .terminals
            .iter()
            .filter(|t| !t.name.starts_with("Newline") && 
    ...
    fn generate_terminal_constructor(&self, terminal: &TerminalInfo) -> TokenStream {
        ...
        let (param_type, param_name, default_value) = match terminal.name.as_str() {
            ...
            "Str" | "Text" | "Code" | "CodeBlock" | "NamedCode" => (quote!(&str), quote!(value), quote!(value)),
