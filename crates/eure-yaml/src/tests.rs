...
fn test_array_to_yaml() {
    let array = Value::Array(Array(vec![Value::I64(1), Value::I64(2), Value::I64(3)]));
...
fn test_variant_external() {
    ...
    match yaml {
        YamlValue::Mapping(map) => {
            assert_eq!(map.len(), 1);
            assert_eq!(
                map.get(YamlValue::String("Success".to_string())),
                Some(&YamlValue::String("OK".to_string()))
            );
        }
        _ => panic!("Expected mapping"),
    }
    ...
    match yaml {
        YamlValue::Mapping(map) => {
            assert_eq!(
                ...
                Some(&YamlValue::String("Response".to_string()))
            );
            assert_eq!(
                map.get(YamlValue::String("message".to_string())),
                Some(&YamlValue::String("Hello".to_string()))
            );
            assert_eq!(
                map.get(YamlValue::String("code".to_string())),
                Some(&YamlValue::Number(200.into()))
            );
        }
        _ => panic!("Expected mapping"),
    }
...
fn test_variant_adjacent() {
    ...
    match yaml {
        YamlValue::Mapping(map) => {
            assert_eq!(
                map.get(YamlValue::String("type".to_string())),
                Some(&YamlValue::String("Error".to_string()))
            );
            assert_eq!(
                map.get(YamlValue::String("content".to_string())),
                Some(&YamlValue::String("Not found".to_string()))
            );
        }
        _ => panic!("Expected mapping"),
    }
...
fn test_variant_untagged() {
    ...
    match yaml {
        YamlValue::Mapping(map) => {
            assert_eq!(map.len(), 1);
            assert_eq!(
                map.get(YamlValue::String("id".to_string())),
                Some(&YamlValue::Number(123.into()))
            );
        }
        _ => panic!("Expected mapping"),
    }
...
fn test_all_variant_representations() {
    ...
    match internal_yaml {
        YamlValue::Mapping(map) => {
            assert_eq!(
                map.get(YamlValue::String("type".to_string())),
                Some(&YamlValue::String("point".to_string()))
            );
    ...
    match adjacent_yaml {
        YamlValue::Mapping(map) => {
            assert_eq!(
                map.get(YamlValue::String("kind".to_string())),
                Some(&YamlValue::String("log-entry".to_string()))
            );
...
fn test_variant_missing_tag_errors() {
    ...
    let mut map = serde_yaml::Mapping::new();
    map.insert(
        YamlValue::String("field1".to_string()),
        YamlValue::String("value1".to_string()),
    );
    map.insert(
        YamlValue::String("field2".to_string()),
...
fn test_invalid_variant_content() {
    ...
    map.insert(
        ...
        YamlValue::String("not a map".to_string()),
    );
...
fn test_conversion_edge_cases() {
    // Empty collections
    assert_eq!(
        yaml_to_value(&YamlValue::Sequence(vec![])).unwrap(),
        Value::Array(Array(vec![]))
    );
    assert_eq!(
        yaml_to_value(&YamlValue::Mapping(serde_yaml::Mapping::new())).unwrap(),
        Value::Map(Map(ahash::AHashMap::new()))
    );
...
fn test_yaml_special_strings() {
    ...
    let maybe_bool_cases = vec![("yes", true), ("no", false), ("on", true), ("off", false)];
    ...
    assert_eq!(
        yaml_to_value(&quoted_true).unwrap(),
...
fn test_unicode_and_escapes() {
    ...
    let mut map = serde_yaml::Mapping::new();
    map.insert(
        YamlValue::String("emoji🎉".to_string()),
        YamlValue::String("value😀".to_string()),
    );
    map.insert(
        YamlValue::String("chinese中文".to_string()),
        YamlValue::String("测试".to_string()),
    );
        ...
        match back {
            Value::F64(result) => {
                if f.is_finite() {
                    ...
                    assert!(
                        relative_error < 1e-10 || (result - f).abs() < 1e-300,
...
fn test_variant_with_null_content() {
    ...
    match yaml {
        YamlValue::Mapping(ref map) => {
            assert_eq!(
                map.get(YamlValue::String("Empty".to_string())),
                Some(&YamlValue::Null)
            );
        }
        _ => panic!("Expected mapping"),
